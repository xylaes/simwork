...
export interface Scenario {
  ...
  statusBadge: 'Active Sprint' | 'Available' | 'Coming Soon';
  manager: {
    name: string;
    title: string;
    avatar: string;
...
Sarah`,
    ...
    stressTestScenario: {
      question: 'If we cap 3rd-party broker dispatches at 10% of total volume next quarter to protect margins, how will we absorb peak holiday freight overflow without missing customer delivery SLAs?',
      context: 'The Chief Commercial Officer is worried that cutting off spot brokers will cause delayed shipments during Thanksgiving/Christmas rush.',
      goodResponseSignals: [
        'Dedicated contract carriers instead of spot market',
        'Drop-and-hook pre-loading',
        'Dynamic SLA tiered pricing',
        'Off-peak night shifts'
      ]
    },
    sampleDispatchData: [
      {
  ...
  {
    id: 'finpulse-staging-blackout',
    title: '502 Gateway Outage & Migration Deadlock',
    companyName: 'FinPulse Payments',
    companyIndustry: 'B2B Fintech & Transaction Infrastructure',
    roleTitle: 'Associate Cloud Infrastructure / SRE',
    sprintDuration: '3-Day Work Sprint (Simulated)',
    difficulty: 'Advanced',
    ...
    briefing: {
      urgentSubject: 'P1 INCIDENT: Payment Webhook Gateway dropping requests across staging/prod',
      receivedTime: 'Friday at 4:18 PM',
      emailBody: `Team,

We are actively hemorrhaging 502s on the merchant webhook ingestion tier following release v3.8.4.

Database pool saturation is pegged at 100%. The on-call engineer attempted a pod restart, but the pods are entering CrashLoopBackOff immediately.

I need an Incident Triage & Root Cause Post-Mortem (RCA) on my desk. Don't just tell me it's "broken"—identify:
1. Which exact migration or commit triggered the deadlock?
2. Why did connection pools fail to drain gracefully?
3. What is the step-by-step rollback and schema remediation plan?

Don't guess. Pull the logs and give me an engineering-grade report.

Marcus`,
      targetObjectives: [
        'Diagnose table-locking DDL statements in migration v3.8.4.',
        'Analyze connection pool starvation across worker pods.',
        'Produce an executive-ready Post-Mortem Incident Report (RCA) with mitigation actions.'
      ],
      keyConstraints: [
        'Cannot drop merchant transaction logs under any circumstance.',
        'Zero-downtime rollback required.'
      ]
    },
    evaluationRubric: {
      keyInsightsExpected: [
        'Identified non-concurrent index creation locking the transactions table.',
        'Recognized database connection pool exhaustion leading to gateway 502s.',
        'Prescribed CREATE INDEX CONCURRENTLY and connection pooling limits.'
      ],
      criticalPitfalls: [
        'Suggesting dropping and recreating the database table.',
        'Failing to identify the DDL locking mechanism.'
      ]
    },
    stressTestScenario: {
      question: 'If we roll back the migration, how do we handle the 1,400 webhook events that were queued and failed with 502 during the 18-minute outage window?',
      context: 'Merchants are demanding immediate reconciliation for unconfirmed payment callbacks.',
      goodResponseSignals: [
        'Idempotent dead-letter queue replay',
        'Exponential backoff consumer workers',
        'Merchant callback deduplication'
      ]
    },
    resumeBulletTemplates: [
      {
        action: 'Triaged P1 Cloud Infrastructure Incident',
        impact: 'resolved database connection starvation causing 18 minutes of 502 gateway downtime across payments API.',
        fullBullet: 'Triaged critical payment API infrastructure outage; diagnosed DDL table-locking deadlock on PostgreSQL cluster and engineered zero-downtime rollback restoring 99.99% service availability.'
      }
    ]
  },
  {
    id: 'healthcare-threat-hunt',
    title: 'EDR Alert: Lateral Movement Triage',
    companyName: 'HealthCare Horizon',
    companyIndustry: 'Regional Hospital Network & Health IT',
    roleTitle: 'Associate SOC & Threat Detection Analyst',
    sprintDuration: '4-Day Work Sprint (Simulated)',
    difficulty: 'Advanced',
    statusBadge: 'Coming Soon',
    manager: {
      name: 'Elena Rostova',
      title: 'Director of Security Operations (CISO)',
      avatar: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?w=150&auto=format&fit=crop&q=80',
      email: 'e.rostova@healthcarehorizon.org',
    },
    briefing: {
      urgentSubject: 'SEV-1 SECURITY ALERT: Anomalous PowerShell beaconing on subnet 10.4.12.0/24',
      receivedTime: '2:14 AM EST',
      emailBody: `Security Team,

CrowdStrike flagged an encoded PowerShell execution originating from a billing clerk workstation. Within 8 minutes, anomalous SMB traffic was detected probing our internal patient record databases.

I need a containment plan and forensic incident brief before 8:00 AM clinical rounds.

1. Did the adversary achieve domain privilege escalation?
2. What systems need immediate network isolation?
3. What IOCs (Indicators of Compromise) must be pushed to our perimeter firewalls?

Elena`,
      targetObjectives: [
        'Analyze SIEM event logs and PowerShell base64 decoding.',
        'Trace lateral movement via Pass-the-Hash / Kerberoasting.',
        'Deliver a CISO Threat Brief with containment and IOC blocklists.'
      ],
      keyConstraints: [
        'Emergency room critical systems cannot be unplugged from power.',
        'Evidence preservation must satisfy HIPAA/HITECH compliance.'
      ]
    },
    evaluationRubric: {
      keyInsightsExpected: [
        'Decoded encoded PowerShell reverse shell payload.',
        'Identified compromised service account credentials.',
        'Scoped isolation to billing VLAN without disrupting clinical telemetry.'
      ],
      criticalPitfalls: [
        'Rebooting the infected host and wiping volatile RAM evidence.',
        'Over-isolating critical care telemetry networks.'
      ]
    },
    stressTestScenario: {
      question: 'The Chief of Medicine is furious that you isolated the billing server because patient admission charts are delayed. How do you justify the containment while mitigating clinical impact?',
      context: 'Hospital executives are threatening to override the security lockout to avoid patient check-in delays.',
      goodResponseSignals: [
        'Proved active ransomware staging risk to life-support systems',
        'Staged read-only emergency paper backup protocol',
        'Fast-tracked micro-segmented scrubbed server clone'
      ]
    },
    resumeBulletTemplates: [
      {
        action: 'Spearheaded SOC Threat Containment Investigation',
        impact: 'neutralized unauthorized lateral movement within healthcare network, preventing ransomware deployment across 4 hospital facilities.',
        fullBullet: 'Led Tier-1/2 incident containment across 1,200 hospital endpoints; decoded encoded PowerShell payloads, isolated compromised billing subnets, and preserved HIPAA-compliant forensic artifacts.'
      }
    ]
  }
];

export const APEX_LOGISTICS_SCENARIO = SCENARIOS[0];
